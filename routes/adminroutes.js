...
// Add new product
router.post("/products", isAdmin, upload.single("image"), handleMulterError, async (req, res) => {
  try {
...
// Update product
router.put("/products/:id", isAdmin, upload.single("image"), handleMulterError, async (req, res) => {
  try {
    const { title, categories, description, price, originalPrice } = req.body;
    ...
    if (req.file) {
      ...
      if (currentProduct && currentProduct.imageUrl && !currentProduct.imageUrl.includes('default-product')) {
